...
png_dir = Path("osx/catalina/alpha/").absolute()
enlarged_glyph_dir = Path("enlarged").absolute()
out_dir = Path("enlarged_alpha/").absolute()
pngs = [png_dir / p.name for p in enlarged_glyph_dir.iterdir() if p.suffix == ".png"]
shuffle(pngs)
